...
const projectedGrid = tslFn(([scale, thickness, offset]) => {
  const uv = projectedGridUv(positionWorld, normalWorld)
    .div(scale)
    .add(thickness.mul(0.5))
    .add(offset)
    .mod(1);
  return max(uv.x.step(thickness), uv.y.step(thickness));
});

const scaleUniform = uniform(0.2);
const thicknessUniform = uniform(1);
...
const gridMaterial = new MeshStandardNodeMaterial();
...
gui
  .addColor(
    { color: colorBackUniform.value.getHexString(THREE.SRGBColorSpace) },
    "color"
  )
  .onChange((value) => {
    colorBackUniform.value.set(value);
  })
  .name("colorBack");
gui
  .addColor(
    { color: colorSmallUniform.value.getHexString(THREE.SRGBColorSpace) },
    "color"
  )
  .onChange((value) => {
    colorSmallUniform.value.set(value);
  })
  .name("colorSmall");
gui
  .addColor(
    { color: colorBigUniform.value.getHexString(THREE.SRGBColorSpace) },
    "color"
  )
  .onChange((value) => {
    colorBigUniform.value.set(value);
  })
  .name("colorBig");
